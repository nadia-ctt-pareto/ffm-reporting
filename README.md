...
- **AI field polish** (Phase 7c, Supabase mode only): "Polish" button on prose fields (summary, win narrative, task title, risk description, etc.) — re-write each field under Foundation First's house voice via BYOK Anthropic key, server-proxied and encrypted at rest. Key never reaches the browser; owner-only RLS with no admin branch. Inline suggestion with Accept/Discard/Undo; original text never touched until Accept. Prevents mid-flight edits from applying stale suggestions.
- **Settings** (`/settings`): **Appearance** theme picker (Light / Dark / System preference that tracks OS); **Prompt Library** (copy-to-clipboard cards for Claude via MCP); **MCP Access** (Phase 8a, Supabase mode only: create/list/revoke bearer tokens for Claude Code/Desktop/CLI); **AI Key** (Phase 7c, Supabase mode only: BYOK Anthropic key entry, fingerprint display); **CSV Import Templates** (downloadable examples with the exact column contract locked in `lib/csv-templates.ts`); **CSV Import** (upload with project targeting, drag-drop, full-file issue accumulation).
- **Claude integration** (Phase 8a, Supabase mode only): remote MCP server at `/api/mcp` with 8 tools (list_reports, get_report, list_projects, get_week_rollup, create_report, update_report, create_project, create_weekly_from_dailies; no delete_report). Claude Code, Desktop, or CLI connects via bearer token (create in Settings). Reads are org-wide (every signed-in teammate's reports); writes are owner-only (user's own rows only). No admin escalation for tokens — even an admin user's token acts as a plain member. The in-repo Skill (`skills/weekly-reports/`) teaches the domain model, merge semantics, workflows, and the no-delete guardrail.
...
**Now** (Phase 8a + 7c complete): Full optional Supabase backend + Claude connectivity. **Demo mode** (no env) runs on `localStorage`, everything local — sidebar shell with SVG nav icons, real dark mode (Light / Dark / System), pagination, full report screen, interactive HTML slide-deck presentation (keyboard/swipe/deep-links), print-to-PDF, Task view (List/Kanban) + Calendar view (Week/Month), daily reports (`/daily`) + weekly wizard roll-up, Settings screen, **Zod schemas** as the single source of truth, **Project entity** for organizing imported reports, **CSV import** with formula-injection safety, **report consolidation** (`/consolidate`). **Supabase mode** (env configured) adds Postgres persistence, per-user ownership via magic-link Auth, **cross-machine share links via public tokens**, local→Postgres import, RLS access control, **Claude integration** via MCP server (8 tools, read org-wide / write owner-only, no admin escalation), and **AI field polish** (BYOK Anthropic key, encrypted server-side, per-field editorial intent).
**Next** (Phase 8b): OAuth for claude.ai (layers on Phase 8a's MCP server, no new tools); surface daily reports in Task view and Calendar (documented Phase 4 follow-up). See CLAUDE.md "Roadmap" for full Phase 8-9 plans.
